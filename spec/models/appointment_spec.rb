...
describe Appointment do
  ...
  context :validations do
    ...
    it 'cannot conflict with another start date' do
      ...
      new_appointment = build(:appointment, start: '2014-09-28 13:44:36') # 2 mins after
      new_appointment.valid?.should be_false
    end

     it 'cannot conflict with a prep_duration before the start date' do
       appointment = create(:appointment)
       new_appointment = build(:appointment, start: '2014-09-28 13:40:36') # 2 mins before start
       new_appointment.valid?.should be_false
     end

     it 'cannot conflict with a post_duration after the start date' do
       appointment = create(:appointment)
       new_appointment = build(:appointment, start: '2014-09-28 13:53:36') # 11 mins after start
       new_appointment.valid?.should be_false
     end

     it 'cannot conflict the prep_duration with the post_duration of another' do
       appointment = create(:appointment)
       new_appointment = build(:appointment, start: '2014-09-28 13:56:36') # 14 mins after start
       new_appointment.valid?.should be_false
     end

     it 'can conflict if allow_conflicts is on' do
       appointment = create(:appointment)
       new_appointment = build(:appointment, start: '2014-09-28 13:45:36') # 3 mins after start
